...
``## [X.Y.Z] (today)`` and sets CITATION.cff ``date-released`` to today (adding it
after ``version:`` if missing). .zenodo.json must not carry a version (Zenodo takes it from
...
def set_date_released(text: str, day: str) -> str:
    if re.search(r"^date-released:", text, flags=re.MULTILINE):
        return replace_field(text, r"^(date-released: *)[^\n]+$", day)
    updated, count = re.subn(r"^(version: *[^\n]+\n)", lambda match: f"{match[1]}date-released: {json.dumps(day)}\n", text, count=1, flags=re.MULTILINE)
    if count != 1:
        raise ValueError("CITATION.cff needs a 'version:' line")
    return updated
