...
- `FastaIndex(path)`: random access to an uncompressed FASTA file by accession. One
  pass records each entry's byte range; `index[acc]` reads only that entry. `write_fai()`
  and `FastaIndex.from_fai()` save and load a samtools-compatible `.fai`. Compressed
  files raise `FastaError`.
